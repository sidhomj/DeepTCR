...
df_plot['perc'] = perc
df_plot['w'] = (np.abs(df_plot['pred']-0.5)*2)**10
...
plt.yticks(np.arange(0.0,y_max+y_max/10,y_max/10))
ax.set_yticklabels(np.round(np.arange(0.0,1.1,0.1),1))
plt.xlabel('')
plt.ylabel('')
plt.scatter(df_plot['pred'],df_plot['w']*y_max,s=2,c='k')
plt.show()
...
w = np.array(df_plot['w'])

file = 'cm038_x2_w.pkl'
featurize = False
if featurize:
    DTCR_U = DeepTCR_U('test_hum', device='/device:GPU:6')
    DTCR_U.Load_Data(beta_sequences=beta_sequences, v_beta=v_beta, d_beta=d_beta, j_beta=j_beta, hla=hla, w=w)
